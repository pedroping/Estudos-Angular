  ...
  ngOnInit(): void {
    ...
    if (this.formcontrol)
      this.formcontrol.valueChanges
        ...
        .subscribe(() => this.validatForm(this.formcontrol!));
  }
    ...
    Object.keys(ERRORS).forEach((key) => {
      ...
      if (key == 'maxlength' || key == 'minlength')
        return this.setInnerHTML(
          `<small>${ERRORS[key].messageFn(Control.errors[key].requiredLength)}</small>`,
        );
      if (key == 'max')
        return this.setInnerHTML(
          `<small>${ERRORS[key].messageFn(Control.errors[key].max)}</small>`,
        );
      if (key == 'min')
        return this.setInnerHTML(
          `<small>${ERRORS[key].messageFn(Control.errors[key].min)}</small>`,
        );
      if (key == 'not_allowed_characters')
        return this.setInnerHTML(
          `<small>${ERRORS[key].messageFn(Control.errors[key][0])}</small>`,
        );
